...
float4 psmain(VPS_INOUT input) : SV_TARGET
{
    ...
    float4 waterColor = float4(0.24, 0.37, 0.49, 1); //GroundMap.Sample(GroundMapSampler, input.texcoord * 100.0f);
    float4 crestColor = float4(1, 1, 1, 1); //CliffMap.Sample(CliffMapSampler, input.texcoord * 60.0f);
